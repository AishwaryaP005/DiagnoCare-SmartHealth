...
# Page config 
st.set_page_config(page_title="AI Diet Type Recommender", 
layout="centered") 
if st.button("Back"): 
...
DIET_INFO = { 
    "Balanced": { 
        "description": "A well-rounded diet with all food groups in 
moderation.", 
...
def create_pdf_report(user_data, diet_info, bmi_fig): 
    ...
    pdf.cell(200, 10, txt=f"BMI: {user_data['bmi']:.1f} 
({user_data['bmi_category']})", ln=1) 
...
condition = st.selectbox("Select your health condition", ["No Condition", 
...
if st.button("Get Diet Recommendation"): 
    ...
    # Summary Card 
    st.markdown(f""" 
    <div style='background-color:#F5F5F5; padding:20px; border
radius:10px;'> 
        <h3 style='color:#007BFF;'>Recommended Diet: <span 
style='color:#28A745;'>{diet_type}</span></h3> 
        <p><strong>BMI:</strong> {bmi:.1f} <span 
style='color:{bmi_color};'>({bmi_category})</span></p> 
