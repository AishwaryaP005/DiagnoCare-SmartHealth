...
def show_page(doctor_name): 
    ...
    selected_patient = st.selectbox("Select Patient to View Reports", 
patients) 
    ...
    if selected_patient: 
        ...
        if diagnosis_reports: 
            for report in diagnosis_reports: 
                ...
                st.markdown(f"**Date**: {diagnosis_date} | **Type**: 
{report_type}") 
                st.download_button("Download Diagnosis Report", 
open(file_path, "rb").read(), 
                                   file_name=os.path.basename(file_path)) 
                if notes: 
                    st.text_area("Clinical Summary", notes, height=150, 
disabled=True) 
                st.markdown("---") 
        else: 
            st.info("No diagnosis reports uploaded by you for this 
patient.") 
        ...
        if patient_files: 
            for rec in patient_files: 
                ...
                st.download_button("Download Patient Report", 
open(file_path, "rb").read(), 
                                   file_name=os.path.basename(file_path)) 
        else: 
...
value=datetime.today()) 
        report_type = st.selectbox("Report Type*", 
                                   ["Lab Results", "Imaging Scan", 
"Clinical Notes", "Prescription", "Other"]) 
        uploaded_file = st.file_uploader("Upload Report File*", 
...
def save_diagnosis(doctor_name, patient_name, diagnosis_date, report_type, 
file, clinical_notes): 
    try: 
        ...
        cursor.execute(""" 
            ...
            (doctor_name, patient_name, diagnosis_date, report_type, 
file_path, clinical_notes) 
            VALUES (%s, %s, %s, %s, %s, %s) 
            """, 
                       (doctor_name, patient_name, diagnosis_date, 
report_type, filepath, clinical_notes) 
                       ) 
        conn.commit() 
