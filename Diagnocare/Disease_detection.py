...
def load_data(): 
    df = pd.read_csv( 
        r"D:\PYTHON MINI PROJECT\datasets\disease_detection dataset 
(kaggle)\archive\Final_Augmented_dataset_Diseases_and_Symptoms.csv" 
    ) 
    return df 
...
if st.button("Submit Symptoms"): 
    if not selected_symptoms: 
        ...
        with st.spinner("Analyzing your symptoms..."): 
            ...
            user_input = np.array([1 if symptom in selected_symptoms else 0 
for symptom in symptom_columns]) 
            ...
            # Compute match score 
            match_counts = np.sum((symptom_matrix == 1) & (user_input == 
1), axis=1) 
            ...
            top_matches = 
matched_df.groupby('diseases')['match_score'].mean().sort_values(ascending=
 False).reset_index() 
...
top_matches.set_index('diseases')['match_score'].plot(kind='barh', ax=ax, 
color='skyblue') 
...
align='C') 
                ...
                for idx, row in top_matches.iterrows(): 
                    pdf.cell(200, 10, txt=f"- {row['diseases']} (Score: 
{row['match_score']:.2f})", ln=True) 
                pdf.ln(10) 
            ...
            pdf_file = create_pdf_report(selected_symptoms, top_matches, 
fig) 
