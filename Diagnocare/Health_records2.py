...
def view_reports(username): 
    try: 
        ...
        # Fetch diagnosis and clinical summary for the patient (added 
feature) 
        cursor.execute(""" 
        ...
        if not patient_reports and not doctor_reports: 
            ...
            if doctor_reports: 
                ...
                for report in doctor_reports: 
                    ...
                    col1, col2 = st.columns([4, 1]) 
                    with col1: 
                        st.write(f"**Diagnosis Report**: 
{os.path.basename(file_path)}") 
                    with col2: 
                        if st.button("View Diagnosis Report", 
key=f"report_{file_path}"): 
                            os.startfile(file_path) 
 
                    if clinical_notes: 
                        st.text_area("Clinical Summary", clinical_notes, 
height=200) 
