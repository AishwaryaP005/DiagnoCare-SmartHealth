...
if uploaded_file is not None: 
    ...
    # 4. Merge coordinates 
    df['Latitude'] = df['States/UTs'].map(lambda x: coordinates.get(x, 
[None, None])[0]) 
    df['Longitude'] = df['States/UTs'].map(lambda x: coordinates.get(x, 
[None, None])[1]) 
 
    df = df.dropna(subset=['Latitude', 'Longitude']) 
 
    # 5. Select Year 
    years = [col for col in df.columns if col not in ['States/UTs', 
'Latitude', 'Longitude']] 
    ...
    with left_col: 
        st.markdown(f"<h4>Heatmap for Year: {selected_year}</h4>", 
unsafe_allow_html=True) 
    ...
    with right_col: 
        ...
        <ul style="line-height:1.5;"> 
            <li><span style="color:#ffffb2;">Light Yellow</span>: Low 
deaths</li> 
            <li><span style="color:#fecc5c;">Orange</span>: Moderate 
deaths</li> 
        ...
        st.markdown("<h5>Search Deaths by Area</h5>", 
unsafe_allow_html=True) 
        search_area = st.text_input("Enter State/UT name (e.g., Bihar)") 
 
        if search_area: 
            result = df[df['States/UTs'].str.lower() == 
search_area.lower()] 
            if not result.empty: 
                deaths = result[selected_year].values[0] 
                st.success(f"{search_area} had **{deaths} deaths** in 
**{selected_year}**.") 
            else: 
