...
class ReportPDF(FPDF): 
    ...
    def footer(self): 
        ...
        self.cell(0, 20, "AI-generated report. Not a substitute for 
professional medical advice.", 0, 0, 'C') 
    ...
    def add_page(self, *args, **kwargs): 
        ...
        self.add_border()  # Automatically add border whenever a new page 
is created 
    ...
    def body(self, image_path, probability, report_text): 
        ...
        self.cell(0, 10, f"Pneumonia Probability: {probability:.2f}%", 
ln=True) 
        self.set_font("Arial", '', 12) 
...
medical advice. " 
                              "Please consult a licensed medical 
professional for any concerns.") 
 
def generate_pneumonia_report(image_path, probability, report_text, 
output_path="pneumonia_report.pdf"): 
    pdf = ReportPDF() 
...
uploaded_file = st.file_uploader("Upload a Chest X-ray Image", type=["png", 
"jpg", "jpeg"]) 
...
if uploaded_file is not None: 
    ...
    if st.button("Predict Pneumonia"): 
        ...
        st.metric(label="Pneumonia Probability", 
value=f"{percentage:.2f}%") 
        ...
        fig = go.Figure(go.Indicator( 
            ...
            gauge={ 
                'axis': {'range': [0, 100]}, 
                'bar': {'color': "#DC3545" if percentage > 50 else 
"#28A745"}, 
                'steps': [ 
        ...
        st.markdown(f"<div style='background-color:#F5F5F5; padding: 15px; 
border-radius: 5px;'>", unsafe_allow_html=True) 
 
        if percentage < 50: 
            risk_text = f"Low Risk Detected ({percentage:.2f}%)" 
            report_summary = ( 
                "- The uploaded scan indicates a **low probability** of 
pneumonia.\n" 
                "- You're likely in the **safe zone**, but always consult a 
doctor if symptoms persist.\n" 
                "- Pneumonia is a lung infection, and early care ensures 
better recovery." 
            ) 
        else: 
            risk_text = f"High Risk Detected ({percentage:.2f}%)" 
            report_summary = ( 
                "- This scan suggests a **high likelihood** of 
pneumonia.\n" 
                "- Please **visit a healthcare provider immediately** for 
further tests and confirmation.\n" 
                "- Pneumonia can lead to serious issues if untreated. Early 
medical care is crucial." 
        ...
        with st.expander("What is Pneumonia?"): 
            st.write(""" 
                Pneumonia is a lung infection that can be mild or life
threatening. 
        ...
        pdf_path = generate_pneumonia_report(temp_img_path, percentage, 
report_summary) 
