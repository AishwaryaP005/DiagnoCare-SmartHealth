...
def send_sms(hospital_name, hospital_phone): 
    try: 
        ...
        message = client.messages.create( 
            body=f"Emergency! Help required at {hospital_name}. Immediate 
assistance needed.", 
...
def make_call(to_number): 
    ...
    call = client.calls.create( 
        twiml='<Response><Say>This is an emergency call from your 
healthcare assistant. Immediate help is needed.</Say></Response>', 
        from_=TWILIO_PHONE_NUMBER, 
        to=to_number 
    ) 
    return call.sid 
...
def main(): 
    ...
    location = st.text_input("Enter your location (e.g., Pune, Maharashtra, 
India):") 
    ...
    if location: 
        ...
        geo_url = 
f"https://api.geoapify.com/v1/geocode/search?text={encoded_location}&apiKey
 ={GEOAPIFY_API_KEY}" 
 
        geo_response = requests.get(geo_url) 
 
        if geo_response.status_code != 200: 
            st.error(f"Failed to get geolocation! (Status Code: 
...
f"https://api.geoapify.com/v2/places?categories=healthcare.hospital&filter=
 circle:{longitude},{latitude},5000&limit=5&apiKey={GEOAPIFY_API_KEY}" 
        ) 
        hospital_response = requests.get(hospital_url) 
 
        if hospital_response.status_code != 200: 
            st.error(f"Failed to get hospitals! (Status Code: 
{hospital_response.status_code})") 
        ...
        for idx, hospital in enumerate(hospital_data['features']): 
            hospital_name = hospital['properties'].get('name', 'Unnamed 
Hospital') 
        ...
        preferred_hospital = st.selectbox( 
            ...
            [h['properties'].get('name', 'Unnamed Hospital') for h in 
hospital_data['features']], 
            key="hospital_select" 
        ) 
 
        phone_number = st.text_input("Enter hospital or emergency phone 
number:", key="phone_input") 
        ...
        with col1: 
            if st.button("Trigger Emergency Call", key="sos_call_button"): 
                if preferred_hospital: 
                    st.success(f"Emergency call triggered for 
{preferred_hospital}!") 
        ...
        with col2: 
            if st.button("✉️ Send Emergency SMS", key="sos_sms_button"): 
                if preferred_hospital and phone_number: 
                    ...
                    st.success(f"Emergency message sent successfully to 
{phone_number}!") 
                    st.toast(f"Emergency SMS sent to 
{preferred_hospital}!", icon="📨") 
